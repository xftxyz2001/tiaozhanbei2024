...
for n in range(1,190):
        sentences2.append(data1.loc[n]['Passage_two'] + str(int(data1.loc[n]['Score_two'])))
i=1
while i < data.shape[0]:
# for i in range(1,data.shape[0]):
    
    while data.loc[i]['Score_two']==3 :
        i=i+1;
    for n in range(1,len(sentences2)):
        sentences1.append(data.loc[i]['Passage_two'])
#     print(sentences1)
...
#     print(max)
    ...
    answer=data.loc[i]['Score_two']
#     print(max)
#     print(max,temp_c,answer)
    if int(sentences2[max][len(sentences2[max])-1:len(sentences2[max])])==data.loc[i]['Score_two']:
        correct=correct+1
    if temp_c >= 3:
