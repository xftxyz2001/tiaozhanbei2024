...
sentences2 = [
    ...
    "我的奶奶，一天，我想去看望生病地奶奶。妈妈同意了，走了好久才到。我在家一直都很像奶奶，一直都看着她地照片，现在终于可以看见她了4",
...
#     print(sentences1)
    ...
    for n in range(len(sentences1)):
        if cosine_scores[n][n]>cosine_scores[max][max]:
            max=n;
    if int(sentences2[max][len(sentences2[max])-1:len(sentences2[max])])==data.loc[i]['Score_one']:
        correct=correct+1
    sentences1=[]
