...
for n in range(1,190):
        sentences2.append(data1.loc[n]['Passage_three'] + str(int(data1.loc[n]['Score_three'])))
i=1
while i < data.shape[0]:
# for i in range(1,data.shape[0]):
    
#     while data.loc[i]['Score_three']==3 or data.loc[i]['Passage_three']=='无':
#         i=i+1;
    while  data.loc[i]['Passage_three']=='无':
        i=i+1;
    print(i)
#     if data.loc[i]['Passage_three']=='无':
#         conti=conti+1
#         continue;
    for n in range(1,len(sentences2)):
        sentences1.append(data.loc[i]['Passage_three'])
#     print(sentences1)
    ...
    for n in range(len(sentences1)):
        if cosine_scores[n][n]>cosine_scores[max][max]:
            max=n;
#     print(max)
    temp_c=int(sentences2[max][len(sentences2[max])-1:len(sentences2[max])])
    answer=data.loc[i]['Score_three']
    print(temp_c,answer,data.loc[i]['Passage_three'])
    if temp_c==answer:
        correct=correct+1
    if temp_c >= 3:
...
print(correct/(data.shape[0]-conti));
