...
from model_1demon_cross import TTpretrainModel
import sys
...
def parse_args():
    ...
    parser.add_argument("--load", type=str, default='./output/epoch1601_01_per0.6.pth')
...
#             continue;
        loss = model(question_emb, multiple_choices_emb, text_emb, feats, multiple_choices, answer, 0.6)
...
#             
        sum = 0;
        sum_c = 0;
#         print(loss[1].size(),max,answer_choice,text)
        
        temp_c=torch.zeros(1);
#         temp_c[0]=loss[8]
        temp_c[0]=loss[5]
#         print(temp_c,answer,text,multiple_choices)
...
#         plt.savefig('3dplot.png', dpi=300)
    print("无的个数",conti)
    print("小项F1", (correct * 100) / ( (turn + 1)-conti), "%")
    print("评分F1", (score_cor * 100) /( (turn + 1)-conti), "%")
