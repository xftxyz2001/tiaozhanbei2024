...
class MultiHeadedTrilinearAttention(nn.Module):
    ...
    def forward(self, v, q, a, v_mask, q_mask, a_mask):
        ...
        * split D(dim) into (H(n_heads), W(width of head)) ; D = H * W
        ...
        num_q = q.size(1)
        num_a = a.size(1)
        # print("之前v.size(),q.size(),a.size()", v.size(), q.size(), a.size())
        ...
        # print("之后v.size(),q.size(),a.size()", v.size(), q.size(), a.size())
        v = v.unsqueeze(0)
        q = q.unsqueeze(0)
        a = a.unsqueeze(0)
...
#         att = self.drop(F.softmax(att.view(num_batch, self.n_heads, -1), -1))
#         att = att.view(num_batch, -1, num_v, num_q, num_a)
        v_out = v_out.unsqueeze(0)
        q_out = q_out.unsqueeze(0)
        a_out = a_out.unsqueeze(0)
#         v_out=v_out.reshape(num_batch,att.size(1),768,-1)
#         q_out = q_out.reshape(num_batch, att.size(1), 768, -1)
#         a_out = a_out.reshape(num_batch, att.size(1), 768, -1)
...
#         print("MultiHeadedTrilinearAttention输出前v.size(),q.size(),a.size()", v_out.size(), q_out.size(), a_out.size())
        return v_out[0], q_out[0], a_out[0]
...
#         print("v.size,v_h.size()",v.size(),v_h.size())
        v = v + v_h
        q = q + q_h
        a = a + a_h
...
class TrilinearUnineTransformer(nn.Module):
    ...
    def forward(self, v, q, a, k, v_mask=None, q_mask=None, a_mask=None, k_mask=None):
        q_i, a_i, v_i, k_i = self.MultiHeadedTrilinearAttentionForimage(q, a, v, k, q_mask, a_mask, v_mask, k_mask)
        q_t, a_t, k_t, v_t = self.MultiHeadedTrilinearAttentionForText(q, a, k, v, q_mask, a_mask, k_mask, v_mask)

        v_i=v_i.reshape(v.size(0),v.size(1))
        v_t = v_t.reshape(v.size(0), v.size(1))
        q_i = q_t.reshape(q.size(0), q.size(1))
        q_t = q_t.reshape(q.size(0), q.size(1))
        a_i = a_i.reshape(a.size(0), a.size(1))
        a_t = a_t.reshape(a.size(0), a.size(1))
        k_i = k_i.reshape(k.size(0), k.size(1))
        k_t = k_t.reshape(k.size(0), k.size(1))
        ...
        for TTblock, Tblock in zip(self.TriTransblocksforimage, self.Transblocksforimage):
            ...
            v_ii_ = Tblock(v_ii_, mask=v_mask)
            q_ii_ = Tblock(q_ii_, mask=q_mask)
            a_ii_ = Tblock(a_ii_, mask=a_mask)
        v_tt = v_ii_+v_ii
        q_tt = q_ii_+q_ii
        a_tt = a_ii_+a_ii
        for TTblock, Tblock in zip(self.TriTransblocksforText, self.TransblocksforText):
            k_tt_, q_tt_, a_tt_ = TTblock(k_ii, q_ii, a_ii, k_mask, q_mask, a_mask)
            k_tt_ = Tblock(k_tt_, mask=k_mask)
            q_tt_ = Tblock(q_tt_, mask=q_mask)
            a_tt_ = Tblock(a_tt_, mask=a_mask)
        q_out = q_tt_ + q_tt
        a_out = a_tt_ + a_tt
        k_out =k_tt_
        v_out = v_tt
...
#         att = F.softmax(att.reshape(num_batch, self.n_heads,-1), -1)
#         att = att.reshape(num_batch, -1, num_v, num_q, num_a)
