...
import torchvision
from TTmodel_ori_nochange import QstEncoder, TrilinearTransformer
# import TTmodel_1demon as TT
import TTmodel_ori_nochange as TT
# ------------------------------
# -------- model ---------------
# ------------------------------
...
#         self.transformer=TT.TrilinearUnineTransformer(num_layers=96, dim=v_dim, num_heads=4, ff_dim=1024, dropout=0)
        self.transformer=TT.TrilinearUnineTransformer(num_layers=4, dim=v_dim, num_heads=4, ff_dim=1024, dropout=0)
...
#         label=torch.zeros(64).to(torch.device('cuda:0'))
#         label[answer_choice]=1
...
#         q_prediction_scores=nn.Sigmoid(self,q_prediction_scores)
        ...
        a_prediction_scores = self.VQ2(img_feature+ans_feature+qst_feature+scribe_feature)[0]
...
#         print(a_prediction_scores,answer)
        ans_loss=los_func_mse(a_prediction_scores,answer)
...
#         result.append(a_loss+b_loss+ans_loss)
        ...
        result.append(ans_loss+label_loss+a_loss)
