...
#         print(index)
        ...
        passage_one=self.data.loc[index]['Passage_one']
...
#         datum = self.quesTestList[index]
        ...
        answer=torch.zeros(1).to(torch.device("cpu"));
        answer[0]=self.data.loc[index]['Score_one'];
...
#         datum = self.quesTestList[index]
        r4 = "\\[]{}.!@#$%^&*()<>?,./;'|~`+_=-"
        cleanr = re.compile('<.*?>')
        text=self.data.loc[index]['Passage_one']
        feats=np.array(self.get_Img(img_id))\
